...
export default function Profile() {
    ...
    const loadProfile = async () => {
        try {
            ...
            // In Next.js we might want to verify how the api throws errors
            // The original code checks for strict error message "Tidak terautentikasi"
            // Adapt as needed or assume API client behavior is same
            if (error instanceof Error && (error.message === "Tidak terautentikasi" || error.message === "Unauthorized")) {
                router.push("/");
                return;
            }
            toast({
                    ...
                    <div className="relative mb-4">
                        <div className="w-24 h-24 rounded-full p-[2px] bg-gradient-to-tr from-primary via-secondary to-primary animate-gradient bg-[length:200%_200%]">
                            <div className="w-full h-full rounded-full bg-black p-[2px]">
                                <img src={avatarImage.src} alt="Profile" className="w-full h-full rounded-full object-cover bg-muted" data-testid="img-avatar" />
                            </div>
                        </div>
